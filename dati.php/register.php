...
if (is_ajax()) {
    ...
    if (!empty($username) && !empty($password)) { //建立连接
        dbInit();
        $sql = "SELECT `id`,`pwd` FROM users WHERE `name` = '$username'";
        if ($ret = query($sql)) {
            $row = mysqli_fetch_assoc($ret); //处理结果集
            $salt='dati';
            ...
            if ($password == $row['pwd']) {//判断密码是否正确
                ...
                $date = date('Y-m-d H:m:s',time());//写入登录表
                $ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
                $info = sprintf("当前访问用户：%s,IP地址：%s,时间：%s /n", $username, $ip, $date);
                echo(retJson(200, '登录成功！'));
            }else{
                echo(retJson(500, '账号密码错误！'));
            }
        }
